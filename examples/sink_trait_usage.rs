...
use quantum_log::{
    ...
    sinks::{
        ...
        traits::{QuantumSink, StackableSink, ExclusiveSink, SinkMetadata, SinkType, SinkError},
        default_stdout::{DefaultStdoutSink, DefaultStdoutConfig, StdoutTarget},
    },
};
...
impl QuantumSink for CustomStackableSink {
    ...
    fn stats(&self) -> String {
        format!("CustomStackableSink: name={}, prefix={}", self.name, self.prefix)
    }

    fn metadata(&self) -> SinkMetadata {
        SinkMetadata::new(self.name.clone(), SinkType::Stackable)
            .with_description(format!("Custom stackable sink with prefix: {}", self.prefix))
    }
}
...
impl QuantumSink for CustomExclusiveSink {
    ...
    async fn shutdown(&self) -> Result<(), Self::Error> {
        println!("[EXCLUSIVE] Shutting down custom exclusive sink: {}", self.name);
        Ok(())
    }
    ...
    fn stats(&self) -> String {
        format!("CustomExclusiveSink: name={}, buffer_size={}", self.name, self.buffer.len())
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 2. 创建自定义 sink
    println!("\n2. 创建自定义 sink");
    let custom_stackable1 = CustomStackableSink::new(
        "Logger1".to_string(),
        "CUSTOM1".to_string()
    );
    let custom_stackable2 = CustomStackableSink::new(
        "Logger2".to_string(),
        "CUSTOM2".to_string()
    );
    ...
    // 添加可叠加型 sink（可以有多个）
    pipeline.add_stackable_sink(custom_stackable1.clone(), custom_stackable1.metadata()).await?;
    pipeline.add_stackable_sink(custom_stackable2.clone(), custom_stackable2.metadata()).await?;
...
mod tests {
    ...
    async fn test_custom_stackable_sink() {
        let sink = CustomStackableSink::new(
            "test".to_string(),
            "TEST".to_string()
        );
    ...
    async fn test_pipeline_with_custom_sinks() {
        ...
        let stackable = CustomStackableSink::new(
            "test_stackable".to_string(),
            "TEST".to_string()
        );
        
        pipeline.add_stackable_sink(stackable.clone(), stackable.metadata()).await.unwrap();
