...
impl FileTools {
    ...
    /// 
    /// 成功时返回文件句柄，失败时返回错误
    pub fn open_file_append<P: AsRef<Path>>(file_path: P, create_if_not_exists: bool) -> Result<File> {
        let file_path = file_path.as_ref();
        ...
        file_path.canonicalize()
            .map_err(|e| QuantumLogError::IoError {
                source: std::io::Error::other(
                    format!("规范化路径失败: {}: {}", file_path.display(), e)
                )
            })
    }
    ...
    pub fn generate_timestamped_filename(base_name: &str, extension: &str) -> String {
        ...
        format!("{}_{}_{}.{}", base_name, timestamp, std::process::id(), extension)
    }
...
mod tests {
    ...
    fn test_file_extension() {
        assert_eq!(FileTools::get_file_extension("test.txt"), Some("txt".to_string()));
        assert_eq!(FileTools::get_file_extension("test.LOG"), Some("log".to_string()));
