...
impl FileWriter {
    ...
    pub async fn new(config: FileConfig) -> Result<Self> {
        ...
        let file = OpenOptions::new()
            ...
            .append(true)
            .open(&path)
            .map_err(|e| QuantumLogError::InternalError(format!("Failed to open file {}: {}", path.display(), e)))?;
        
        let writer = BufWriter::new(file);
        let current_size = tokio::fs::metadata(&path).await
            .map(|m| m.len())
            .unwrap_or(0);
    ...
    pub async fn write(&self, data: &[u8]) -> Result<()> {
        ...
        // 写入数据
        {
            ...
            writer.write_all(data)
                .map_err(|e| QuantumLogError::InternalError(format!("Failed to write to file: {}", e)))?;
    ...
    async fn rotate(&self) -> Result<()> {
        ...
        if self.archive {
            ...
            if FileTools::file_exists(&self.current_path) {
                tokio::fs::rename(&self.current_path, &archive_path).await
                    .map_err(|e| QuantumLogError::InternalError(format!("Failed to archive file: {}", e)))?;
            }
        }
        ...
        let file = OpenOptions::new()
            ...
            .truncate(true)
            .open(&new_path)
            .map_err(|e| QuantumLogError::InternalError(format!("Failed to create new file {}: {}", new_path.display(), e)))?;
    ...
    fn generate_archive_path(&self) -> Result<PathBuf> {
        ...
        let file_stem = current_path.file_stem()
            .and_then(|s| s.to_str())
            .unwrap_or("quantum");
...
impl FileCleaner {
    ...
    pub async fn cleanup(&self) -> Result<usize> {
        ...
        let mut entries = tokio::fs::read_dir(&self.base_path).await
            .map_err(|e| QuantumLogError::InternalError(format!("Failed to read directory: {}", e)))?;
        
        let mut files = Vec::new();
        while let Some(entry) = entries.next_entry().await
            .map_err(|e| QuantumLogError::InternalError(format!("Failed to read directory entry: {}", e)))? {
...
mod tests {
    ...
    async fn test_file_rotation_by_size() {
        ...
        // 这证明轮转成功发生了
        assert_eq!(final_size, large_data.len() as u64, 
                   "Expected final_size ({}) == data_len ({})", final_size, large_data.len());
        ...
        // 应该有2个文件：当前文件和归档文件
        assert!(file_count >= 2, "Expected at least 2 files (current + archived), found {}", file_count);
    }
