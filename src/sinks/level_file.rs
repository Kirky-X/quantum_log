...
use crate::sinks::file_common::{FileWriter, FilePathGenerator, FileCleaner};
    ...
    pub async fn send_event(&self, event: QuantumLogEvent) -> Result<()> {
        if let Some(sender) = &self.sender {
            ...
            sender.send(message).await
                .map_err(|_| QuantumLogError::SinkError("Failed to send event to LevelFileSink".to_string()))?;
        }
        Ok(())
    }
...
impl LevelFileSinkProcessor {
    ...
    async fn get_or_create_writer(&mut self, level: &str) -> Result<&mut FileWriter> {
        if !self.writers.contains_key(level) {
            ...
            let file_config = crate::config::FileConfig {
                ...
                directory: file_path.parent().unwrap_or_else(|| std::path::Path::new(".")).to_path_buf(),
                filename_base: file_path.file_stem().unwrap_or_else(|| std::ffi::OsStr::new("log")).to_string_lossy().to_string(),
    ...
    fn should_log_level(&self, level: &str) -> bool {
        if let Some(ref levels) = self.config.levels {
            levels.contains(&level.parse::<crate::config::LogLevel>().unwrap_or(crate::config::LogLevel::Info))
        } else {
            true
        }
    }
...
mod tests {
    use super::*;
    use crate::config::{LogLevel, OutputFormat, RotationPolicy};
    ...
    async fn test_json_format() {
        ...
        let file_content = tokio::fs::read_to_string(temp_dir.path().join("INFO.log")).await.unwrap();
